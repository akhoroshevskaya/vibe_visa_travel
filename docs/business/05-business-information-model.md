...
Service не принадлежит конкретному Case.
...
Необходимо определить, требуется ли отдельная сущность **Case Service**.

На текущем этапе связь между Case и Service считается прямой.

После построения Business Entity Relationship Diagram будет принято окончательное архитектурное решение.
