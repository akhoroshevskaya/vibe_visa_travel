...
**Business Entity:** BE-006 Workflow Template
...
Репозиторий **Workflows** хранит **шаблоны бизнес-процессов (Workflow Templates)**, которые определяют порядок выполнения услуг компании.

Workflow Template является многоразовым описанием процесса и может использоваться любым количеством Product и Case.

При создании нового Case система автоматически создает **Workflow** — экземпляр выполнения процесса, основанный на выбранном Workflow Template.

Workflow принадлежит конкретному Case, хранит текущее состояние выполнения, историю прохождения этапов и не является объектом данного репозитория.

Таким образом:

- **Workflow Template** — описывает, *как должен выполняться процесс*;
- **Workflow** — отражает, *как выполняется конкретный Case*.

Данный репозиторий хранит только Workflow Templates и не хранит экземпляры выполнения Workflow.
